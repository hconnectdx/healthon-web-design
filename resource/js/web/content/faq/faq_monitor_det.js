...
//입력필드 썸머노트적용
    ...
    $("#new_faq_thumb_Dropzone").dropzone({
        ...
        init : function(){
            ...
            this.on("addedfile", function(file) {
                ...
                let sno = $('form#faqForm #thumbFileSno').val();
                ...
                if (currentFileSize > 0) {
                    ...
                    $("#error-alert-modal #e_faq").html(i18n('management.video.txt.limit'));
                }
            });
    ...
    var faqSno = $('form#faqForm #faqSno').val();
...
function applyFaq(faqSno) {
    ...
    var enTitle = $("#faqForm #titleFaq10801100").val();
    var enFaq = $("#faqForm input[name=faqContents10801100]").val();
    var enResgiter = $("#faqForm input[name=faqWriterNm10801100]").val();
    ...
    if(isEmpty($('#new_faq_thumb_Dropzone')[0].dropzone.files[0])) {
        //썸네일 이미지 필수등록 체크
        $("#error-alert-modal").modal('show');
        $("#e_faq").html(i18n('management.faq.null.pop.txt.image.check'));
        return false;
    } else if(isEmpty(enTitle)) {
        //영어제목 필수등록 체크
        $("#error-alert-modal").modal('show');
        $("#e_faq").html(i18n('management.faq.null.pop.txt.title.eng'));
        return false;
    } else if(isEmpty(enResgiter)) {
        //영어작성자 필수등록 체크
        $("#error-alert-modal").modal('show');
        $("#e_faq").html(i18n('management.faq.null.pop.txt.register.eng'));
        return false;
    } else if(isEmpty(enFaq)) {
        //영어내용 필수등록 체크
        $("#error-alert-modal").modal('show');
        $("#e_faq").html(i18n('management.faq.null.pop.txt.txt.eng'));
        return false;
        ...
        $.ajax({
            ...
            success : function(data){
                ...
                $("#success-alert-modal #faq").html(i18n('management.faq.modify.pop.txt.txt'));
        ...
        $.ajax({
            ...
            success: function (data) {
                ...
                $("#success-alert-modal #faq").html(i18n('management.faq.new.pop.txt.txt'));
                // $("#new_contents").modal('hide');
                //처음 생성할때 ajax만 새로고침
                $('#faq-datatable').DataTable().ajax.reload();
                updateFaqContent();
